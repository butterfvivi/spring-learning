<template>
  <div v-loading="loading" class="report-show-contain" style="display: flex;flex-direction: column;">
    ...
    <div style="flex: 1;overflow: hidden;position: relative;">
      ...
      <div id="FloatImagePanel" style="position: absolute;width: 0;height: 0;top: 10px;left: 10px;">
        <img v-for="floatImage in floatImages" v-if="!floatImage.onlyFirstPage || pageIndex==1" :src="floatImage.value" :width="floatImage.width" :height="floatImage.height" :style="{position: 'absolute',top: (floatImage.top - scrollTop) + 'px',left: (floatImage.left - scrollLeft) + 'px'}"  />
      </div>
    </div>
    <el-dialog
      title="预览"
  ...
  methods: {
    ...
    search() {
      ...
      loadData(param,this.token).then(response => {
        ...
        this.chartDatas = []
        this.floatImages = data.floatImages || []
        ...
        this.$nextTick(function(){
          ...
          for(const chartData of chartDatas) {
            ...
            if(ele) {
              ...
              ele.style.height = parentHeight + 'px'
    ...
    handleScroll() {
      ...
      this.scrollTop = centerPanel.scrollTop || 0
      this.scrollLeft = centerPanel.scrollLeft || 0
