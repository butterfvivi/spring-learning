<template>
  <el-form class="div-block-clearance" label-position="left" label-width="90px" :model="form" size="mini">
    ...
    <el-form-item label="系列属性:">
      <el-select
        v-model="form.seriesProperty"
        placeholder="请选择"
        clearable
        style="width:100%"
        @change="handleSeriesProperty"
      >
        <el-option
          v-for="item in fieldOptions"
          :key="item.name"
  ...
  methods: {
    ...
    refresh(rowIndex, colIndex, row2Index, col2Index) {
      ...
      this.form = {
        ...
        seriesProperty: dataset.seriesProperty,
        format: dataset.format,
        seriesFormat: dataset.seriesFormat,
    ...
    handleSeriesProperty(val) {
      const dataset = this.cellDef.value.chart.dataset
      dataset.seriesProperty = val
      this.chartValue.context.refresh(this.cellDef)
    },
    add() {
      this.oldRow = null
