<template>
  <div>
    ...
    <el-tabs v-model="activeName" stretch>
      ...
      <el-tab-pane label="过滤" name="five">
        <condition-dialog :conditions="conditions" :fields="properties" />
      </el-tab-pane>
      <el-tab-pane label="字典" name="third">
        <el-tabs stretch>
          <el-tab-pane label="分类属性数据映射">
            <dictionary-editor :data-mapping="dict" />
          </el-tab-pane>
          <el-tab-pane label="系列属性数据映射">
            <dictionary-editor :data-mapping="seriesDict" />
          </el-tab-pane>
        </el-tabs>
      </el-tab-pane>
...
export default {
  ...
  methods: {
    ...
    refresh(rowIndex, colIndex, row2Index, col2Index) {
      ...
      // 系列属性数据映射
      if (!chart.dataset.seriesDict) {
        chart.dataset.seriesDict = {}
      }
      this.seriesDict = {
        datasets: datasets,
        cellDef: {
          value: chart.dataset.seriesDict
        }
      }
      // 选中数据集字段
