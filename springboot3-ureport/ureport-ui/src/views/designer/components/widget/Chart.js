  ...
  const chartOptions = {
    ...
    tooltip: {
      ...
      formatter: function(param) {
        return `${param.marker}<span class="chart-tooltip-label">${param.name}</span><span class="chart-tooltip-value">${param.data[1]}(${param.percent}%)</span>`
      }
    },
    legend: setLegend(legend),
    series: [{
...
function builderScatter(chart) {
  const {options, dimensions, indicators, dataset }  = chart
  const {title, grid, xAxis, yAxis, legend, label }  = options
  const [legendTop, legendLeft] = legend.position.split('-')
  const chartOptions = {
    dataset: dataset || [],
    ...
    tooltip: {
      ...
      appendToBody: true,
      formatter: function(param) {
        let content = '<div style="font-size:14px;color:#666;font-weight:400;line-height:1;">' + param.data[0] + '</div>'
        content = content + '<div style="margin: 10px 0 0;">'
        for(var i = 0; i < indicators.length; i++) {
          content = content + '<div style="margin: 5px 0 0;">'
          content = content + param.marker
          content = content + '<span style="font-size:14px;color:#666;font-weight:400;margin-left:2px">' + indicators[i] + '</span>'
          content = content + '<span style="float:right;margin-left:20px;font-size:14px;color:#666;font-weight:900">' + param.data[i + 1] + '</span>'
          content = content + '</div>'
        }
        content = content + '</div>'
        return content
  ...
  if(chart.type === 'bubble') {
    ...
    for(var k = 0; k < dataset.length;k++) {
      ...
      for(var m = 0; m < source.length;m++) {
        ...
        min = Math.min(min,value[3])
        max = Math.max(max,value[3])
      }
    }
    const arr = chartOptions.series
    for(var k = 0; k < arr.length;k++) {
      ...
      item.symbolSize = function (data) {
        ...
        return ((data[3] - min) * (50 - 5)) / (max - min) + 5
      }
    }
  }
  return chartOptions
