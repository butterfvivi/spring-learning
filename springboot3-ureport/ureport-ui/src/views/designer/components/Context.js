...
export default class Context {
  ...
  selectionCellRange() {
    ...
    if(selected) {
      var rowIndex = Math.min(selected[0],selected[2])
      var colIndex = Math.min(selected[1],selected[3])
      var row2Index = Math.max(selected[0],selected[2])
      var col2Index = Math.max(selected[1],selected[3])
      return {rowIndex,colIndex,row2Index,col2Index}
    }
    return null
  }
  ...
  refreshSelected() {
    const selected = this.hot.getSelected()
    if (!selected) return
    Handsontable.hooks.run(this.hot, "afterSelectionEnd", ...selected)
  }
