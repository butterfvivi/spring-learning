<template>
  <div class="component-icon-btn" style="height: 24px;">
    <div class="component-dropdown-group component-icon-btn-icon" style="float: left;" @click="handleForeColor">
      <i class="icons icons-16 icons-16-font_color" />
      <div class="component-icon-btn-color" :style="{backgroundColor:color}" />
    </div>
    <el-dropdown ref="toolForeColorDropdown" size="mini" trigger="click" @visible-change="handleDropdownMenu">
...
export default {
  ...
  methods: {
    ...
    handleForeColor() {
      ...
      let oldForecolorStyle = updateCellsForecolorStyle(context, rowIndex, colIndex, row2Index, col2Index, color)
      undoManager.add({
        redo: function() {
          oldForecolorStyle = updateCellsForecolorStyle(context, rowIndex, colIndex, row2Index, col2Index, color)
          setDirty()
        },
        undo: function() {
