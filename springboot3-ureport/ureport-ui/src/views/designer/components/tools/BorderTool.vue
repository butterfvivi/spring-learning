<template>
  <div class="component-icon-btn" style="height: 24px;">
    ...
    <el-dropdown ref="borderDropdownMenu" trigger="click" @visible-change="handleVisibleChange">
            ...
            <div @click="handleColorDropdownMenu">
              <span class="el-dropdown-link">
                <div class="component-icon-btn-icon" style="float: left;">
                  ...
                  <div class="component-icon-btn-color" :style="{backgroundColor:borderColor,top:'120px',left: '14px'}" />
                </div>
              ...
              <transition name="el-zoom-in-top">
                <div v-if="colorDropdownMenu" class="color_dropdown-menu" style="cursor: default;" @click.stop="">
                  <table>
                    <tbody>
...
export default {
  ...
  methods: {
    ...
    handleBorder() {
      ...
      let oldBorderStyle = updateBorderStyles(context, rowIndex, colIndex, row2Index, col2Index, newBorder, borderType)
      undoManager.add({
        redo: function() {
          oldBorderStyle = updateBorderStyles(context, rowIndex, colIndex, row2Index, col2Index, newBorder, borderType)
          setDirty()
        },
        undo: function() {
  ...
  for (let i = startRow; i <= endRow; i++) {
    for (let j = startCol; j <= endCol; j++) {
      ...
      if (target === 'full') {
        ...
        const { rowspan, colspan } = getSpan(context.hot, i, j)
        if (i === startRow) {
          cellStyle.topBorder = Object.assign({}, newBorder)
        }
        if (i + (rowspan ? rowspan - 1 : 0) === endRow) {
          cellStyle.bottomBorder = Object.assign({}, newBorder)
        }
        if (j === startCol) {
          cellStyle.leftBorder = Object.assign({}, newBorder)
        }
        if (j + (colspan ? colspan - 1 : 0) === endCol) {
          cellStyle.rightBorder = Object.assign({}, newBorder)
        }
