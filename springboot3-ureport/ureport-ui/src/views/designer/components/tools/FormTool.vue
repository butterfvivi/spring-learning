              ...
              <el-tab-pane label="属性" name="second">
                ...
                <el-form v-if="row.uuid" :model="row" label-width="80px" label-position="left" size="small" style="padding: 0px 10px;">
                  ...
                  <el-form-item label="绑定参数:">
                    <g-autocomplete
                      v-model="row.bindParameter"
                      style="width:100%"
                      :placeholder="'绑定参数'"
                      :data="valueBindParameters"
                      @change="handleBindParameter"
                    />
...
export default {
  ...
  methods: {
    ...
    handleBindParameter(val) {
      this.row.bindParameter = val
    },
    remove(ev, index) {
    ...
    openForm() {
      ...
      for (const ds of datasources) {
        ...
        for (const t of d) {
          ...
          for (var i = 0; i < parameters.length; i++) {
            let p = parameters[i]
            if(!valueBindParameters.find(e => e.value === p.name)) {
              valueBindParameters.push({value: p.name})
            }
          }
          datasets.push({
            name: t.name,
      ...
      this.valueBindParameters = valueBindParameters
