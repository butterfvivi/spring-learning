...
import { cleanCopyPasteDirty } from './table/operation/CopyPasteCellOperation.js'
...
export async function tableToXml(context) {
  ...
  for (let i = 0; i < countRows; i++) {
    for (let j = 0; j < countCols; j++) {
      ...
      for (let pc of propertyConditions) {
        ...
        if (pc.newValue !== null && pc.newValue !== undefined) {
  ...
  for (let datasource of datasources) {
    ...
    if (type === 'jdbc') {
      ...
      for (let dataset of datasource.datasets) {
        ds += `<dataset name="${encode(dataset.name)}" method="${dataset.method}" type="api">`;
        for (let field of dataset.fields) {
          ds += `<field name="${field.name}"/>`;
        }
        for (let parameter of dataset.parameters) {
  ...
  xml +=
    ...
    left-margin="${paper.leftMargin}"
    right-margin="${paper.rightMargin}"
    top-margin="${paper.topMargin}"
    bottom-margin="${paper.bottomMargin}"
    page-enabled="${paper.pageEnabled}"
    paging-mode="${paper.pagingMode}"
    page-type="${paper.pageType || 'system'}"
    page-size="${paper.pageSize || 20}"
    dataset-name="${paper.datasetName || ''}"
    property="${paper.property || ''}"
    fixrows="${paper.fixRows}"
    width="${paper.width}"
    height="${paper.height}"
    orientation="${paper.orientation}"
    html-report-align="${paper.htmlReportAlign}"
    bg-image="${paper.bgImage || ''}"
    print="${paper.print || false}"
    html-interval-refresh-value="${htmlIntervalRefreshValue}"
    column-enabled="${paper.columnEnabled}"`;
...
async function buildSlashCell(td, value, name) {
  ...
  if(value.svgXml) {
    ...
    div.style.width = value.style.width
    div.style.height = value.style.height
    div.innerHTML = value.svgXml
    document.body.appendChild(div)
    const svgs = div.getElementsByTagName("svg")
    if (svgs && svgs.length > 0) {
      slashValue = await saveSvgAsPng.svgAsPngUri(svgs[0], {
        encoderOptions: 1
      })
    }
...
function buildChartCell(value, name) {
  ...
  if(dataset.type !== 'combo' && dataset.type !== 'pie' && dataset.type !== 'doughnut') {
    if (dataset.seriesProperty) {
      cellXml += ` series-property="${dataset.seriesProperty}"`
    }
    if (dataset.seriesFormat) {
      cellXml += ` series-format="${dataset.seriesFormat}"`
    }
  }
  cellXml += `>`
  ...
  if (dataset.seriesDict) {
    const seriesDict = dataset.seriesDict
    const mappingType = seriesDict.mappingType || 'simple'
    cellXml += `<series-dict mapping-type="${mappingType}"`;
    if (mappingType === 'dataset') {
      cellXml +=
        ` mapping-dataset="${seriesDict.mappingDataset}" mapping-key-property="${seriesDict.mappingKeyProperty}" mapping-value-property="${seriesDict.mappingValueProperty}"`;
    }
    cellXml += `>`;
    if (mappingType === 'simple') {
      const mappingItems = seriesDict.mappingItems;
      if (mappingItems && mappingItems.length > 0) {
        for (let mappingItem of mappingItems) {
          cellXml +=
          `<mapping-item value="${encode(mappingItem.value)}" label="${encode(mappingItem.label)}"/>`;
...
function buildFloatImage() {
  let xml = ''
  const parentElement = document.getElementById('float_image_block')
  const nodes = parentElement.childNodes
  if(nodes && nodes.length > 0) {
    for (let i = 0; i < nodes.length; i++) {
      const node = nodes[i]
      const childNodes = node.childNodes
      const childNode = childNodes[childNodes.length - 2]
      let type = node.getAttribute("data-target-type")
      let source = node.getAttribute("data-target-source")
      xml = xml + `<image width="${node.offsetWidth}" height="${node.offsetHeight}" type="${type}" source="${source}" top="${node.offsetTop - 26}" left="${node.offsetLeft - 50}">`
      xml = xml + `<![CDATA[${childNode.src}]]>`
      xml = xml + `</image>`
    }
  }
  return xml
