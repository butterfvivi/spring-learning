...
import org.vivi.framework.ureport.simple.ureport.build.cell.NoneExpandBuilder;
import org.vivi.framework.ureport.simple.ureport.build.cell.down.DownExpandBuilder;
import org.vivi.framework.ureport.simple.ureport.build.cell.right.RightExpandBuilder;
import org.vivi.framework.ureport.simple.ureport.build.paging.BasePagination;
import org.vivi.framework.ureport.simple.ureport.build.paging.Page;
import org.vivi.framework.ureport.simple.ureport.build.paging.PagingBuilder;
...
public class ReportBuilder extends BasePagination {
	...
	private final static DownExpandBuilder downExpandBuilder = new DownExpandBuilder();
	
	private final static RightExpandBuilder rightExpandBuilder = new RightExpandBuilder();
	
	private final static NoneExpandBuilder noneExpandBuilder = new NoneExpandBuilder();

	private final static HideRowColumnBuilder hideRowColumnBuilder = new HideRowColumnBuilder();
	...
	public Report buildReport(ReportDefinition reportDefinition, Map<String, Object> parameters) {
		...
		ToolUtils.logToConsole("报表计算耗时:" + (end - start) + "ms");
		return report;
	}
		...
		for (Cell cell : cells) {
			...
			if (size == 1) {
				...
				if(Expand.Down.equals(cell.getExpand())) {
					lastCell = downExpandBuilder.buildCell(dataList, cell, context);
				} else if(Expand.Right.equals(cell.getExpand())) {
					lastCell = rightExpandBuilder.buildCell(dataList, cell, context);
				} else if(Expand.None.equals(cell.getExpand())) {
					lastCell = noneExpandBuilder.buildCell(dataList, cell, context);
				}
			}
	...
	private void recomputeCells(Report report, Context context) {
		...
		if (pagingMode.equals(PagingMode.fitpage)) {
			...
			if(!paper.isColumnEnabled()) {
				titleRowHeight = report.getTitleRowsHeight();
			}
