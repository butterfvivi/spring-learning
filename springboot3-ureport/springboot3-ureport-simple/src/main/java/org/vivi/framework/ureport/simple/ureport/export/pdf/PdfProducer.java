 ...
 * the License.
 ******************************************************************************/
package org.vivi.framework.ureport.simple.ureport.export.pdf;
...
import org.vivi.framework.ureport.simple.ureport.export.FullPageData;
...
import org.vivi.framework.ureport.simple.ureport.font.FontBuilder;
...
public class PdfProducer implements Producer {
		...
		try {
			...
			if (list.size() > 0) {
				...
				for (List<Page> pages : list) {
					...
					addFloatImages(report, document, writer, isFirstPage);
					document.newPage();
	...
	private void addFloatImages(Report report, Document document, PdfWriter writer, boolean isFirstPage) {
		...
		if (floatImageList != null && floatImageList.size() > 0) {
			try {
				for (FloatImage floatImage : floatImageList) {
					...
					com.itextpdf.text.Image image = buildPdfImage(url);
					image.scaleAbsolute(UnitUtils.pixelToPoint(floatImage.getWidth().floatValue()), UnitUtils.pixelToPoint(floatImage.getHeight().floatValue()));
					float f1 = document.leftMargin() + Double.valueOf(UnitUtils.pixelToPoint(floatImage.getLeft())).floatValue();
					float f2 = Double.valueOf(UnitUtils.pixelToPoint(floatImage.getTop())).floatValue();
					float f3 = f2 + UnitUtils.pixelToPoint(floatImage.getHeight().floatValue()) + document.topMargin();
					float f4 = document.getPageSize().getHeight() - f3;
					image.setAbsolutePosition(f1, f4);
					pdfContentByte.addImage(image);
				}
			} catch (Exception exception) {
				throw new RuntimeException("bugImg" + exception.getMessage());
			}
		}
	}
}
