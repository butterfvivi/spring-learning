...
public class CellCoordinateExpression extends CellExpression {
	...
	private List<Cell> buildLeftCells(Cell cell, Context context) {
		...
		for (CellCoordinate coordinate : leftCoordinates) {
			...
			if (targetLeftCell == null) {
				if (coordinate.getCoordinateType().equals(CoordinateType.relative)) {
					cellList = ToolUtils.fetchTargetCells(cell, context, name);
				} else {
					cellList = cellsMap.get(name);
				}
			} else {
				cellList = targetLeftCell.getRowChildrenCells(name);
				if(cellList == null) {
					System.out.println("未找到当前单元格的子单元格数据");
		...
		List<Cell> leftCellList = targetLeftCell.getRowChildrenCells(cellName);
		return leftCellList;
	}
	...
	private List<Cell> buildTopCells(Cell cell, Context context) {
		...
		for (CellCoordinate coordinate : topCoordinates) {
			...
			if (cellList == null) {
				...
				cellList = targetTopCell.getColumnChildrenCells(name);
				if(cellList == null) {
					System.out.println("未找到当前单元格的子单元格数据");
					cellList = new ArrayList<Cell>();
				}
		...
		List<Cell> topCellList  = targetTopCell.getColumnChildrenCells(cellName);
		return topCellList;
	}
	...
	public void fetchCellName(List<String> list) {
		...
		if(leftCoordinate != null) {
			List<CellCoordinate> cellCoordinate = leftCoordinate.getCellCoordinates();
			if(cellCoordinate != null && cellCoordinate.size() > 0) {
				StringBuilder sb = new StringBuilder();
				for (CellCoordinate c : cellCoordinate) {
		...
		if(topCoordinate != null) {
			List<CellCoordinate> cellCoordinate = topCoordinate.getCellCoordinates();
			if(cellCoordinate != null && cellCoordinate.size() > 0) {
				StringBuilder sb = new StringBuilder();
				for (CellCoordinate c : cellCoordinate) {
					sb.append(c.getCellName()).append(":");
