...
public class EqualsAssertor extends AbstractAssertor {
	...
	public boolean eval(Object left, Object right) {
		...
		if (left instanceof Number && right instanceof Number) {
			BigDecimal b1 = ToolUtils.toBigDecimal(left);
			BigDecimal b2 = ToolUtils.toBigDecimal(right);
			return b1.compareTo(b2) == 0;
		} else if (left instanceof Number) {
			BigDecimal b1 = ToolUtils.toBigDecimal(left);
			BigDecimal b2 = null;
			try {
				b2 = ToolUtils.toBigDecimal(right);
			} catch (Exception ex) {
			}
			if (b2 != null) {
				return b1.compareTo(b2) == 0;
			...
			BigDecimal b1 = ToolUtils.toBigDecimal(right);
			BigDecimal b2 = null;
			try {
				b2 = ToolUtils.toBigDecimal(left);
			} catch (Exception ex) {
			}
			if (b2 != null) {
				return b1.compareTo(b2) == 0;
