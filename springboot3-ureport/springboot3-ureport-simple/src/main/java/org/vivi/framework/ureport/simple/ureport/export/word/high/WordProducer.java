 ...
 * the License.
 ******************************************************************************/
package org.vivi.framework.ureport.simple.ureport.export.word.high;

import static org.vivi.framework.ureport.simple.ureport.utils.ColorUtils.toHex;
...
import org.vivi.framework.ureport.simple.ureport.definition.BorderStyle;
...
import org.vivi.framework.ureport.simple.ureport.export.word.DxaUtils;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.ChartOption;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.WordAreaChart;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.WordAreaStackedChart;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.WordBarChart;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.WordBarStackedChart;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.WordBubbleChart;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.WordChart;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.WordComboChart;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.WordDoughnutChart;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.WordHistogramChart;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.WordHistogramStackedChart;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.WordLineChart;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.WordLineStackedChart;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.WordPieChart;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.WordRadarChart;
import org.vivi.framework.ureport.simple.ureport.export.word.chart.WordScatterChart;
...
//			}
				...
				for (int rowNumber = 0; rowNumber < rows.size(); rowNumber++) {
					...
					for (Column col : columns) {
						...
						if(rowNumber == 0 && colNumber == 0) {
							XWPFRun run = getXWPFRun(tableCell);
							if(run != null) {
								boolean isFirstPage = pageIndex == 1; 
				    			buildFloatImages(report, run, isFirstPage);
				    		}
					    }
					}
				}
	...
	private void buildTableCellStyle(XWPFTable table, XWPFTableCell tableCell, Cell cell, int rowNumber,int columnNumber,
		...
		if (value instanceof String) {
			...
			try {
				int width = 0;
				int col_span = Math.max(1, colSpan);
				for (int m = columnNumber; m < colWidths.length && m < columnNumber + col_span; m++) {
					width = width + colWidths[m];
				}
				int height = 0;
				int row_span = Math.max(1, rowSpan);
				for (int m = rowNumber; m < rowHeights.length && m < rowNumber + row_span; m++) {
			...
			run.setText(dt.format(localDateTime));
		} else if(value instanceof LocalDateTime) {
			LocalDateTime d = (LocalDateTime) value;
			DateTimeFormatter dt = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
			run.setText(dt.format(d));
		}
	...
	private void buildFloatImages(Report report, XWPFRun run, boolean isFirstPage) {
		List<FloatImage> floatImageList = report.getFloatImages();
		if (floatImageList == null || floatImageList.size() == 0) {
			return;
		}
		InputStream input = null;
		try {
			for (FloatImage floatImage : floatImageList) {
				if(floatImage.isOnlyFirstPage() && !isFirstPage) {
					continue;
				}
				String url = floatImage.getValue();
				input = ImageUtils.getImage(url);
				// 获取图片高宽
				run.addPicture(input, 5, "", Units.toEMU(floatImage.getWidth()), Units.toEMU(floatImage.getHeight()));
				IOUtils.closeQuietly(input);
				// 2. 获取到图片数据
				CTDrawing[] drawingArray = run.getCTR().getDrawingArray();
				CTDrawing drawing = drawingArray[drawingArray.length - 1];
				...
				// 拿到新插入的图片替换添加CTAnchor 设置浮动属性 删除inline属性
				CTAnchor anchor = getAnchorWithGraphic(graphicalobject, "",
						Units.toEMU(UnitUtils.pixelToPoint(floatImage.getWidth())),
						Units.toEMU(UnitUtils.pixelToPoint(floatImage.getHeight())), // 图片大小
						Units.toEMU(UnitUtils.pixelToPoint(floatImage.getLeft())),
						Units.toEMU(UnitUtils.pixelToPoint(floatImage.getTop())), false);// 相对当前段落位置 需要计算段落已有内容的左偏移
				drawing.setAnchorArray(new CTAnchor[] { anchor });// 添加浮动属性
				drawing.removeInline(0);// 删除行内属性
			}
