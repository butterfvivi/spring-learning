...
public class XSSFExcelParser extends ExcelParser {
	...
	public ReportDefinition parse(InputStream inputStream) throws Exception {
		...
		for (int i = 0; i < maxColumnCount; i++) {
			CTCol col = columnHelper.getColumn(i, false);
			double width = 9.0;// 默认宽度
			if(col != null && col.isSetWidth()) {
				width = col.getWidth();
			}
			ColumnDefinition columnDefinition = new ColumnDefinition();
			columnDefinition.setWidth(Double.valueOf(width * 6.0).intValue());
			columnDefinition.setColumnNumber(i + 1);
			columnDefs.add(columnDefinition);
		}
		Paper paper = new Paper();
		XSSFPrintSetup setup = sheet.getPrintSetup();
		paper.setLeftMargin(Double.valueOf(setup.getLeftMargin() * 10.0 / 0.352778).intValue());
		paper.setRightMargin(Double.valueOf(setup.getRightMargin() * 10.0 / 0.352778).intValue());
