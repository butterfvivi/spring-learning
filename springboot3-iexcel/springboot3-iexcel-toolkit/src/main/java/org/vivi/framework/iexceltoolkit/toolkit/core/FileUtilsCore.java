...
import org.vivi.framework.iexceltoolkit.toolkit.dto.AnalysisZipFileDataDto;
import org.vivi.framework.iexceltoolkit.toolkit.dto.DownloadFileDto;
import org.vivi.framework.iexceltoolkit.toolkit.dto.DownloadFileZipDto;
...
public class FileUtilsCore {
    ...
    private static final String NO_PATH = "请传入文件地址！";
    private static final String NO_SUFFIX = "请指定下载的文件后缀！";
    private static final String NO_FILE = "文件不存在！";
    private static final String NO_FILE_LIST = "无文件，无法进行打包下载！";
    private static final String NO_FILE_LIST_SUFFIX = "打包下载，需要定义好每个文件的后缀！";
    private static byte[] buffer = new byte[1024 * 1024 * 10];
    private static final String NO_ANALYSIS_TYPE = "解析.zip压缩包中的文件内容，需要指定解析文件格式，多个文件以逗号拼接，例如：json,txt,xml";
    ...
    public void downloadFileZip(HttpServletResponse response, DownloadFileZipDto zipDto) throws Exception {
        ...
        AssertUtil.objIsNull(dtoList, NO_FILE_LIST);
        //文件后缀
        if (dtoList.stream().filter(t -> EmptyUtils.isEmpty(t.getSuffix())).collect(Collectors.toList()).size() != 0) {
            AssertUtil.throwInnerException(NO_FILE_LIST_SUFFIX);
        }
    ...
    public Map<String, Object> analysisZipFileData(AnalysisZipFileDataDto dto) throws Exception {
        ...
        AssertUtil.objIsNull(path, NO_PATH);
        String analysisType = dto.getAnalysisType();
        AssertUtil.objIsNull(analysisType, NO_ANALYSIS_TYPE);
        //返回值
...
//        }
    ...
    private static InputStream getFileInputStream(DownloadFileDto dto) throws Exception {
        ...
        //绝对路径下载，比如D:\home\1.txt
            ...
            if (!file.exists()) AssertUtil.throwInnerException(NO_FILE + "请检查路径【" + path + "】下是否有所需文件！");
            inputStream = new FileInputStream(file);
        }
        if (inputStream == null) {
            AssertUtil.throwInnerException("无法根据路径【" + path + "】获取文件流！请检查有没有指定路径类型参数pathType值？0-url , 1-resources , 2-绝对路径");
        }
        return inputStream;
    }
    ...
    private static void checkDownloadParams(DownloadFileDto dto) {
        if (EmptyUtils.isEmpty(dto.getPath())) AssertUtil.throwInnerException(NO_PATH);
        if (EmptyUtils.isEmpty(dto.getSuffix())) AssertUtil.throwInnerException(NO_SUFFIX);
    }
