...
//		//分类轴标(X轴),标题位置
		...
		XDDFCategoryDataSource category = XDDFDataSourcesFactory.fromArray((String[]) axisData.toArray(new String[axisData.size()]));
...
//		            chart.getCTChart().getPlotArea().getBarChartArray(0).getSerArray(i).getDLbls().addNewShowSerName().setVal(false);
    ...
    public static void createPie(XSSFSheet sheet,JSONObject chartCell,JSONObject chartOptions,int isCoedit){
		...
		XDDFCategoryDataSource category = XDDFDataSourcesFactory.fromArray((String[]) names.toArray(new String[names.size()]));
    ...
    public static void createRadar(XSSFSheet sheet,JSONObject chartCell,JSONObject chartOptions,int isCoedit){
		...
		XDDFCategoryDataSource category = XDDFDataSourcesFactory.fromArray((String[]) indicator.toArray(new String[indicator.size()]));
