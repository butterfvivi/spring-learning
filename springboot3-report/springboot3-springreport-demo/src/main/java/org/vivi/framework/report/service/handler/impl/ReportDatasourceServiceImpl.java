...
import org.vivi.framework.report.service.mapper.reportdatasource.ReportDatasourceMapper;
...
//           redisUtil.del(RedisPrefixEnum.DATASETCOLUMN.getCode()+String.valueOf(ids.get(i)));
       ...
       result.setStatusMsg("info.delete");
       return result;
   }
...
//		}
       ...
       if (reportDatasource == null) {
           throw new BizException(StatusCode.FAILURE, "error.check.notexist");
       }
...
//           }
           ...
           {
               throw new BizException(StatusCode.FAILURE, "error.notnull");
           }
       ...
       if(StringUtil.isNotEmpty(requestResult)) {
           if("ObjectArray".equals(reportDatasource.getApiResultType())) {
               ...
               try {
                   ...
                   e.printStackTrace();
                   throw new BizException(StatusCode.FAILURE, "error.jsonarrayparse");
               }
               ...
               try {
                   ...
                   e.printStackTrace();
                   throw new BizException(StatusCode.FAILURE, "error.jsonparse");
               }
           }
       }
       return result;
