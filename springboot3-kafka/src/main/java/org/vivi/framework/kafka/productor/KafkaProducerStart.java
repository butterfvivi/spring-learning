package org.vivi.framework.kafka.productor;
...
public class KafkaProducerStart {
    ...
    public Map<String, Object> sendMessage(@RequestParam String topic, @RequestBody Map<String, Object> message) {
        log.info("向指定主题发送信息，topic={},message={}", topic, message);
        try {
    ...
    public Map<String, Object> sendMsgDefault(@RequestBody Map<String, Object> message) {
        log.info("向默认主题发送信息，topic={},topic={}", kafkaTemplate.getDefaultTopic(), message);
        try {
            String valueAsString = new ObjectMapper().writeValueAsString(message);
    ...
    public Map<String, Object> sendMessageCallback(@RequestParam String topic,
        ...
        try {
            ...
            kafkaTemplate.send(topic, valueAsString).thenAcceptAsync(success -> {
                String topic2 = success.getRecordMetadata().topic();
                int partition = success.getRecordMetadata().partition();
                long offset = success.getRecordMetadata().offset();
                log.info("发送消息成功，topic={},partition={},offset={}", topic2, partition, offset);
            },failure -> {
                log.warn("消息发送失败：{}" + failure);
                log.warn("保存到数据库中，后期再做处理.");
            });
        } catch (JsonProcessingException e) {
            e.printStackTrace();
        }
        log.info("向指定主题发送信息，回调，topic={},message={}", topic, message);
        return message;
    }
...
//                            log.warn("消息发送失败：{}，{}", failure.getMessage(), failure);
//                            log.warn("保存到数据库中，后期再做处理.");
//                        }
//                    });
        } catch (JsonProcessingException e) {
    ...
    public Map<String, Object> sendMessageTransactional1(@RequestParam String topic,
                                                         @RequestBody Map<String, Object> message) {
        try {
            log.info("向指定主题发送信息，带事务管理，topic={},message={}", topic, message);
