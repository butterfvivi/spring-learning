package org.vivi.framework.canal.utils;
...
import org.vivi.framework.canal.conf.RedisClient;
import org.vivi.framework.canal.model.User;
...
public class CanalUtil {
    ...
    @Value("${canal-monitor.host}")
    String canalMonitorHost;

    @Value("${canal-monitor.port}")
    ...
    private  void printColumn(List<CanalEntry.Column> columns) {
        ...
        for (CanalEntry.Column column : columns) { //一行数据库数据=一个对象
            ...
            if(column.getName().equalsIgnoreCase("id")){
                ...
                user.setAge(Integer.valueOf(column.getValue()));
        ...
        if(user.getId()!=null && !"".equals(user.getId())){
            ...
            redisClient.set(user.getId(),userJson);//保存用户数据
        }
